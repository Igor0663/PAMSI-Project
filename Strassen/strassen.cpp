...
template<typename T>
matrix<T> strassen_mult(matrix<T> A, matrix<T> B, unsigned int pow)
{
	if(pow == 1)
	{
		return A * B;
	...
	matrix<T> M1 = strassen_mult(A11 + A22, B11 + B22, pow/2);
	matrix<T> M2 = strassen_mult(A21 + A22, B11, pow/2);
	matrix<T> M3 = strassen_mult(A11, B12 - B22, pow/2);
	matrix<T> M4 = strassen_mult(A22, B21 - B11, pow/2);
	matrix<T> M5 = strassen_mult(A11 + A12, B22, pow/2);
	matrix<T> M6 = strassen_mult(A21 - A11, B11 + B12, pow/2);
	matrix<T> M7 = strassen_mult(A12 - A22, B21 + B22, pow/2);
...
matrix<T> strassen( matrix<T> A, matrix<T> B)
{
	...
	{
		std::cerr << "Dimensions don't match! Cannot multiply";
		return A;
	}
	unsigned int max_dim = std::max( std::max(A.size().first, A.size().second), std::max(B.size().first, B.size().second));
	unsigned int pow2 = 1;
	std::pair<unsigned int, unsigned int> C_dim = {A.size().first, B.size().second};
